<script>
	...
	let cmdTitle = 'player@nwp-cgn.de:# ~'
	let pokemon

	let pwMode = false
	const password = 'xxxx'
	const passwordDE = 'xxxx'
	let usrPwInput = ''
...
<main class="main" style="background-color: #2e3440;">
	<div
		id="mainWindow"
		class="w-full font-mono text-[#8FBCBB] leading-tight px-[20px] py-[15px] overflow-x-hidden"
		use:clickOutside
		on:click={() => {
			if (pwMode) {
				pwInput.focus()
		...
		{#if !pwMode}
			<div class="flex border border-dotted">
				<span>{cmdTitle}</span>
				<input
					bind:value={cmd}
					bind:this={input}
